...
def calculate_arc_midpoint(start, end, angle_deg, normal = None):
    ...
    angle_rad = np.radians(abs(angle_deg))              #角度转弧度（圆心角） angle_deg>0为逆时针圆弧，<0为顺时针圆弧
    radius = length / (2 * np.sin(angle_rad / 2))       #半径长度
...
def build_path():"""
    ...
    #path = cq.Workplane("XY").moveTo({path_vertices[0][0]}, {path_vertices[0][1]})\n"""
...
cq.show_object(model)  # 在CQ-Editor中显示
"""
